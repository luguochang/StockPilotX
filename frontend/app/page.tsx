      ...
      >
        ...
        <div className="hero-video-content">
          ...
          <Paragraph style={{ margin: 0, color: "rgba(255,255,255,0.86)", maxWidth: 640 }}>
            首屏使用视频背景和品牌级文案表达，下面直接承接股票分析工作流与数据证据，让视觉与业务同时成立。
          </Paragraph>
          <Space>
          ...
          <Col xs={24} xl={14}>
            <Card className="premium-card" style={{ borderColor: "rgba(15,23,42,0.12)" }}>
              <Space direction="vertical" style={{ width: "100%" }}>
                <Text style={{ color: "#475569" }}>输入标的和问题，触发实时+历史联合分析</Text>
                <StockSelectorModal
                  value={stockCode}
          ...
          <Col xs={24} xl={12}>
            <Card className="premium-card" title={<span style={{ color: "#0f172a" }}>双股票快速对比</span>}>
              <Space direction="vertical" style={{ width: "100%" }}>
                <StockSelectorModal
                  value={compareCode}
                  onChange={(next) => setCompareCode(Array.isArray(next) ? (next[0] ?? "") : next)}
                  title="选择对比标的"
